...
CFLAGS	:=	-g -Wall -O3 -mword-relocations -finline-functions -faggressive-loop-optimizations \
...
all: $(BUILD) $(GFXBUILD) $(DEPSDIR) $(ROMFS_T3XFILES) $(T3XHFILES)
	...
	@echo
	@echo Building cia...
	@$(BANNERTOOL) makesmdh -s $(APP_TITLE) -l $(APP_DESCRIPTION) -p $(APP_AUTHOR) -i $(APP_ICON) -o $(CURDIR)/$(BUILD)/icon.icn
	@$(BANNERTOOL) makebanner -ci $(BANNER_IMAGE) -a $(BANNER_AUDIO) -o $(BUILD)/banner.bnr
	@$(MAKEROM) -f cia -o $(OUTPUT).cia -target t -exefslogo $(MAKEROM_ARGS) -ver $(APP_VER) -icon $(CURDIR)/$(BUILD)/icon.bin -banner $(CURDIR)/$(BUILD)banner.bin
