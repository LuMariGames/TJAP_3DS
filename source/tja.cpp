			...
			if (isStart && isCourseMatch) {
				...
				if (tja_notes[tja_cnt][0] == '#') {
					...
					case COMMAND_SUDDEN:
						...
						break;
					case COMMAND_LYRIC:
						strncpy(ly, Command.value_s, sizeof(ly));
						break;
				...
				strncpy(Measure[MeasureCount].lyric, ly, sizeof(Measure[MeasureCount].lyric));
				...
				if (tja_notes[tja_cnt][0] == '#') {
					...
					case COMMAND_BRANCHSTART:
						...
						strncpy(Beforely, ly, sizeof(Beforely));
				...
				if (isNoComma == false && NotesCount != 0 && tja_notes[tja_cnt][0] != '#') {	//複数行小節の最後の行
					...
					for (int i = 1, j = MeasureCount - Measure[MeasureCount].firstmeasure + 1; i < j; ++i) {	//judge_timeの調整
						...
						if (tja_notes[Measure[MeasureCount].notes][0] != '#') {	//複数行小節の最初の小節以外
							...
							Measure[Measure[MeasureCount].firstmeasure + i].create_time = Measure[Measure[MeasureCount].firstmeasure + i].judge_time + (isSudden ? (240.0 / NextBpm - sudntime) : 0) - (240.0 * NOTES_JUDGE_RANGE) / (Measure[Measure[MeasureCount].firstmeasure + i].bpm * (NOTES_AREA * fabs(Measure[Measure[MeasureCount].firstmeasure + i].scroll * Option.speed)));
...
void get_command_value(char* buf, COMMAND_T *Command) {
	...
	if (buf[0] == '#') {
		...
		if (strcmp(command, "START") == 0) Command->knd = COMMAND_START;
			...
			if (buf[7] != '\n' && buf[7] != '\r') strlcpy(Command->value_s, buf + 7, strlen(buf) - 8);
			else strlcpy(Command->value_s, "", 0);
		}
