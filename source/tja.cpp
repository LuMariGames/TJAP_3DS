...
int tja_cnt = 0, MeasureMaxNumber = 0, stme, redCdn[3];
				...
				if (isNoComma == false && NotesCount != 0 && tja_notes[tja_cnt][0] != '#') {	//複数行小節の最後の行
					...
					for (int i = 1; i < MeasureCount - Measure[MeasureCount].firstmeasure + 1; i++) {	//judge_timeの調整
						...
						if (tja_notes[Measure[MeasureCount].notes][0] != '#') {	//複数行小節の最初の小節以外
							...
							Measure[Measure[MeasureCount].firstmeasure + i].create_time = Measure[Measure[MeasureCount].firstmeasure + i].judge_time - (60.0 / Measure[Measure[MeasureCount].firstmeasure + i].bpm * 4)*(1.0*NOTES_JUDGE_RANGE / (NOTES_AREA*Measure[Measure[MeasureCount].firstmeasure + i].scroll));
