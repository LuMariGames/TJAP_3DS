	...
	if((fp=fopen(abs_path,"r"))!=NULL){
		...
		while((fgets(tja_notes[tja_cnt],NOTES_MEASURE_MAX,fp)!=NULL || tja_cnt<MEASURE_MAX)&&
			...
			if(isStart && isCourseMatch){
				...
				Measure[MeasureCount].create_time=Measure[MeasureCount].judge_time+(isSudden ?(240.0/NextBpm-sudntime): 0)-(240.0*(420.0-NOTES_JUDGE_X))/(Measure[MeasureCount].bpm*(NOTES_AREA*fabs(scroll)));
				...
				if(isNoComma==false && NotesCount!=0 && tja_notes[tja_cnt][0]!='#'){	//複数行小節の最後の行
					...
					for(int i=1,j=MeasureCount-Measure[MeasureCount].firstmeasure+1;i<j;++i){	//judge_timeの調整
						...
						if(tja_notes[Measure[MeasureCount].notes][0]!='#'){	//複数行小節の最初の小節以外
							...
							Measure[Measure[MeasureCount].firstmeasure+i].create_time=Measure[Measure[MeasureCount].firstmeasure+i].judge_time+(isSudden ?(240.0/NextBpm-sudntime): 0)-(240.0 * (420.0-NOTES_JUDGE_X))/(Measure[Measure[MeasureCount].firstmeasure+i].bpm *(NOTES_AREA * fabs(Measure[Measure[MeasureCount].firstmeasure+i].scroll)));
