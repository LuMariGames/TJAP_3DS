...
void notes_main(bool isDon, bool isKatsu, char tja_notes[MEASURE_MAX][NOTES_MEASURE_MAX], MEASURE_T Measure[MEASURE_MAX], int cnt, C2D_Sprite  sprites[SPRITES_NUMER]) {
	...
	if (cnt >= 0) CurrentTimeNotes = get_current_time(TIME_NOTES) + ((Option.measure == 0) ? Measure[stme].create_time : Measure[MinMeasureCount].judge_time);
		...
		while (Measure[MeasureCount].create_time <= CurrentTimeNotes && !Branch.wait) {
			...
			for (int i = 0; i < NotesCount; ++i) {
				...
				if (id != -1 && ctoi(tja_notes[Measure[MeasureCount].notes][i]) != 0 && Measure[MeasureCount].branch == Branch.course) {
					...
					if (MeasureCount < MinMeasureCount && knd == NOTES_BALLOON && Measure[MeasureCount].branch != -1) ++BalloonCount[1]; ++BalloonCount[2]; ++BalloonCount[3];
...
inline void notes_judge(double CurrentTimeNotes, bool isDon, bool isKatsu, int cnt, int branch) {
	...
	if (Option.isAuto) {	//オート
		...
		if (isKatsu && CurrentJudgeNotes[1] != -1) {	//カツ
		...
		if (JudgeBalloonState != -1 && isDon) {	//風船
...
void draw_condition() {
	...
	for (int j = 0; j < 4; ++j) {
		...
		else if (strcmp(exam[j][0], "s") == 0) draw_condition_text(50, 154+20*j, Text[get_lang()][TEXT_NUM_SCORE], &width, &height);
		else if (strcmp(exam[j][0], "r") == 0) draw_condition_text(50, 154+20*j, Text[get_lang()][TEXT_NUM_ROLL], &width, &height);
