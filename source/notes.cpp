...
void notes_judge(void* notejudge) {

	while(get_notes_finish() == false) {

		OPTION_T Option;
		get_option(&Option);
		struct notejudge_t* njt = (notejudge_t*)notejudge;
		...
		for (int i = 0, j = ROLL_MAX - 1; i < j; ++i) {
			...
			if (RollNotes[i].flag && !Notes[RollNotes[i].start_id].isDummy &&
				Notes[RollNotes[i].start_id].judge_time < njt->time &&
				(RollNotes[i].end_id == -1 || (RollNotes[i].end_id != -1 && Notes[RollNotes[i].end_id].judge_time > njt->time))) JudgeRollState = RollNotes[i].knd;
		}
		...
		for (int i = 0, j = BALLOON_MAX - 1; i < j; ++i) {
			...
			if (BalloonNotes[i].flag && !Notes[BalloonNotes[i].start_id].isDummy &&
				Notes[BalloonNotes[i].start_id].judge_time <= njt->time) {
				JudgeBalloonState = i;
				break;
			}
		}
		if (JBS != JudgeBalloonState && JudgeBalloonState != -1) {
			...
			if (balloon[njt->bth][BalloonCount[njt->bth]] > 0) BalloonNotes[JudgeBalloonState].need_hit = balloon[njt->bth][BalloonCount[njt->bth]];
			else BalloonNotes[JudgeBalloonState].need_hit = 5;
			if (njt->bth == 0) ++BalloonCount[0];
			else {
				++BalloonCount[1];
				++BalloonCount[2];
				++BalloonCount[3];
		...
		if (Option.isAuto) {	//オート
			...
			for (int i = 0, j = Notes.size() - 1; i < j; ++i) {
				...
				if (Notes[i].flag && !Notes[i].isDummy && Notes[i].judge_time <= njt->time &&
					Notes[i].isThrough == false && Notes[i].knd < NOTES_ROLL) {

					switch (Notes[i].knd) {
					case NOTES_DON:
						play_sound(SOUND_DON);
						make_judge(PERFECT, njt->time);
						break;
					case NOTES_BIGDON:
						play_sound(SOUND_DON);
						make_judge(SPECIAL_PERFECT, njt->time);
						break;
					case NOTES_KATSU:
			...
			if (JudgeRollState != -1) {	//連打
				...
				if (njt->count % Option.rollspeed <= 0) {
			...
			for (int j = 0,sd = 0,sk = 0,gn = ((njt->donc >= njt->katsuc) ? njt->donc : njt->katsuc); j < gn; ++j) {
				...
				for (int i = 0, j = Notes.size(); i < j; ++i) {
					...
					if (!Notes[i].isDummy && Notes[i].flag) {
						...
						if (Notes[i].knd == NOTES_DON ||
							...
							if (CurrentJudgeNotesLag[0] > fabs(Notes[i].judge_time - njt->time) ||
								CurrentJudgeNotesLag[0] == -1) {
		
								CurrentJudgeNotes[0] = i;
								CurrentJudgeNotesLag[0] = fabs(Notes[i].judge_time - njt->time);
							}
						}
						if (Notes[i].knd == NOTES_KATSU ||
							...
							if (CurrentJudgeNotesLag[1] > fabs(Notes[i].judge_time - njt->time) ||
								CurrentJudgeNotesLag[1] == -1) {
		
								CurrentJudgeNotes[1] = i;
								CurrentJudgeNotesLag[1] = fabs(Notes[i].judge_time - njt->time);
							}
						}
					}
				}
				...
				bool isBig,isBomb;
				if ((njt->donc > sd && Notes[CurrentJudgeNotes[0]].knd == NOTES_BIGDON) || (njt->katsuc > sk && Notes[CurrentJudgeNotes[1]].knd == NOTES_BIGKATSU)) isBig = true;
				else isBig = false;
				if ((njt->donc > sd && Notes[CurrentJudgeNotes[0]].knd == NOTES_BOMB) || (njt->katsuc > sk && Notes[CurrentJudgeNotes[1]].knd == NOTES_BOMB)) isBomb = true;
				else isBomb = false;
		
				if (njt->donc > sd && CurrentJudgeNotes[0] != -1) {	//ドン
					...
					if (CurrentJudgeNotesLag[0] <= Option.judge_range_perfect) {			//良
						...
						delete_notes(CurrentJudgeNotes[0]);
						if (isBig) {
							make_judge(SPECIAL_NICE, njt->time);
				...
				if (njt->katsuc > sk && CurrentJudgeNotes[1] != -1) {	//カツ
			...
			if (JudgeRollState != -1) {	//連打
				...
				for (int i = 0, j = (njt->donc + njt->katsuc); i < j; ++i) {
					if (JudgeRollState == NOTES_ROLL) update_score(ROLL);
					else if (JudgeRollState == NOTES_BIGROLL) update_score(BIG_ROLL);
				}
			}
			...
			int dc = 0,kc = 0;
			while (JudgeBalloonState != -1 && (dc < njt->donc || kc < njt->katsuc)) {	//風船

				if (Notes[BalloonNotes[JudgeBalloonState].start_id].knd == NOTES_DENDEN && kc < njt->katsuc && isDendenCH != 0) {
					++BalloonNotes[JudgeBalloonState].current_hit;
					++kc;
					isDendenCH = 0;
					if (BalloonNotes[JudgeBalloonState].current_hit >= BalloonNotes[JudgeBalloonState].need_hit) {
				...
				else if (Notes[BalloonNotes[JudgeBalloonState].start_id].knd == NOTES_DENDEN && dc < njt->donc && isDendenCH <= 0) {
					++BalloonNotes[JudgeBalloonState].current_hit;
					++dc;
					isDendenCH = 1;
					if (BalloonNotes[JudgeBalloonState].current_hit >= BalloonNotes[JudgeBalloonState].need_hit) {
				...
				else if (Notes[BalloonNotes[JudgeBalloonState].start_id].knd != NOTES_DENDEN && dc < njt->donc) {
					++BalloonNotes[JudgeBalloonState].current_hit;
					++dc;
					if (BalloonNotes[JudgeBalloonState].current_hit >= BalloonNotes[JudgeBalloonState].need_hit) {
...
void notes_main(int isDon,int isKatsu,char tja_notes[MEASURE_MAX][NOTES_MEASURE_MAX],MEASURE_T Measure[MEASURE_MAX],int cnt,C2D_Sprite sprites[SPRITES_NUMER]) {
	...
	if (cnt == 0) {
		...
		notejudge.time = CurrentTimeNotes;
		notejudge.donc = isDon;
		notejudge.katsuc = isKatsu;
		notejudge.count = cnt;
		notejudge.bth = ((Branch.course == -1) ? 0 : Branch.course - 11);
		thread = threadCreate(notes_judge, (void*)notejudge, 8192, 0x18, 0, true);
	}
		...
		if (Notes[i].flag) {
			...
			case NOTES_POTATO:
				...
				if (Notes[i].x != NOTES_JUDGE_X) {
					...
					C2D_DrawImage(sprites[SPRITE_POTATO].image, &sprites[SPRITE_POTATO].params, &DummyTint);
				}
				else if (BalloonNotes[Notes[i].roll_id].current_hit <= BalloonNotes[Notes[i].roll_id].need_hit) {
