...
extern int isBranch, comboVoice, course, isBadCondition, stme, black;
double bpm, offset;
	...
	for (int i = 0; i < BARLINE_MAX - 1; i++) {
		...
		if (BarLine[i].flag == true) {
			...
			BarLine[i].x = BarLine[i].x_ini -
				NOTES_AREA * BarLine[i].scroll * (CurrentTimeNotes - Measure[BarLine[i].measure].pop_time) / (60.0 / Measure[BarLine[i].measure].bpm * 4);
	...
	for (int i = 0; i < NOTES_MAX - 1; i++) {	//計算
		...
		if (Notes[i].flag == true) {
			...
			Notes[i].x = Notes[i].x_ini -
				NOTES_AREA * Notes[i].scroll * (CurrentTimeNotes - Notes[i].pop_time) / (60.0 / Notes[i].bpm * 4);
...
void draw_condition_text(float x, float y, const char *text, float *width, float *height) {
	...
	C2D_DrawText(&NotesText, C2D_WithColor, x, y, 1.0f, size, size, C2D_Color32f(1.0f, 1.0f, 1.0f, 1.0f));
}
