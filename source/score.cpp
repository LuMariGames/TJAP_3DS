...
void calc_base_score(MEASURE_T Measure[MEASURE_MAX], char notes[MEASURE_MAX][NOTES_MEASURE_MAX]) {	//初項と公差を計算　魂ゲージの伸びも
	...
	case 6:		//段位道場
		...
		if (exam[0][0] == "g" && exam[0][3] == "m") Gauge.norma = Gauge.soul * (redCdn[0] / 100.00);
		else if (exam[1][0] == "g" && exam[1][3] == "m") Gauge.norma = Gauge.soul * (redCdn[1] / 100.00);
		else if (exam[2][0] == "g" && exam[2][3] == "m") Gauge.norma = Gauge.soul * (redCdn[2] / 100.00);
		else if (exam[3][0] == "g" && exam[3][3] == "m") Gauge.norma = Gauge.soul * (redCdn[3] / 100.00);
		break;
	}
	//init_score_after();
}
