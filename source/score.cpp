...
TotalPerfectCount,TotalNiceCount,TotalBadCount,TotalScore,TotalRollCount,TotalBombCount,
...
void draw_gauge(C2D_Sprite  (&sprites)[SPRITES_NUMER]){
	...
	//赤
	C2D_DrawRectSolid(123,76,0,250.0*(float)(Gauge.norma/Gauge.soul),8,C2D_Color32f(102.0/255,0,0,1));
	C2D_DrawRectSolid(123,76,0,250.0*gauge,8,C2D_Color32f(1,0,0,1));

	//黄
	C2D_DrawRectSolid(123+250.0*(float)(Gauge.norma/Gauge.soul),67,0,250-250.0*(float)(Gauge.norma/Gauge.soul),17,C2D_Color32f(102.0/255,68.0/255,0,1));
	if(250*gauge-(250.0*Gauge.norma/Gauge.soul)>= 0)
		C2D_DrawRectSolid(123+250.0*(float)(Gauge.norma/Gauge.soul),67,0,250*gauge-(250.0*(float)(Gauge.norma/Gauge.soul)),17,C2D_Color32f(1,1,12.0/255,1));
...
void draw_gauge_result(C2D_Sprite (&sprites)[SPRITES_NUMER]){
	...
	//赤
	C2D_DrawRectSolid(x_start,76,0,x_end*(float)(Gauge.norma/Gauge.soul),8,C2D_Color32f(102.0/255,0,0,1));
	C2D_DrawRectSolid(x_start,76,0,x_end*gauge,8,C2D_Color32f(1,0,0,1));

	//黄
	C2D_DrawRectSolid(x_start+x_end*(float)(Gauge.norma/Gauge.soul),67,0,x_end-x_end*(float)(Gauge.norma/Gauge.soul),17,C2D_Color32f(102.0/255,68.0/255,0,1));
	if(x_end*gauge-(x_end*Gauge.norma/Gauge.soul)>= 0)
		C2D_DrawRectSolid(x_start+x_end*(float)(Gauge.norma/Gauge.soul),67,0,x_end*gauge-(x_end*(float)(Gauge.norma/Gauge.soul)),17,C2D_Color32f(1,1,12.0/255,1));
...
int dan_condition(){
	...
	const char* types[]={"jb","jg","jp","s","r","h","c","g","jm"};
	uint64_t counts[]={TotalBadCount,TotalNiceCount,TotalPerfectCount,TotalScore,TotalRollCount,TotalCount,MaxComboCount,(uint64_t)(Gauge.score/Gauge.soul*100.0),TotalBombCount};
	...
	for(int i=0;i<9;++i){
		...
		(strcmp(exam[3][0],types[i])==0&&strcmp(exam[3][3],"l")==0&&counts[i]>=Cdn[1][3])){
			isBadCondition=0;
			i=99;
		}
	}
	...
	for(int i=0;i<9;++i){
		...
		(strcmp(exam[3][0],types[i])==0&&strcmp(exam[3][3],"l")==0&&counts[i]>=Cdn[0][3]))++isBadCondition;
	}
	...
	if(isSpecial()&&TotalBadCount>0)isBadCondition=9;
...
void calc_base_score(MEASURE_T (&Measure)[MEASURE_MAX],char (&notes)[MEASURE_MAX][NOTES_MEASURE_MAX]){	//初項と公差を計算　魂ゲージの伸びも
	...
	if((TJA_Header.scoreinit==-1||TJA_Header.scorediff==-1)&&(scoremode==1||scoremode==2)){	//新配点と旧配点
		...
		int scoretmp=0;
		while(scoretmp<TmpBaseCeilingPoint&&combo>0)scoretmp+=combo*10;
		init=scoretmp/combo;
		diff=0;
	}
	else if(scoremode==0){
	...
	case 7:		//エンドレス段位
		...
		else if(strcmp(exam[2][0],"g")==0&&strcmp(exam[2][3],"m")==0 )Gauge.norma=Gauge.soul *(Cdn[0][2]/100.00);
		else if(strcmp(exam[3][0],"g")==0&&strcmp(exam[3][3],"m")==0 )Gauge.norma=Gauge.soul *(Cdn[0][3]/100.00);
		break;
	}
	//init_score_after();
}
...
void draw_emblem(C2D_Sprite  (&sprites)[SPRITES_NUMER]){
	...
	case COURSE_ENDLESSDAN:
		C2D_DrawRectSolid(0,86,0,62,58,C2D_Color32f(1,195.0/255.0,6.0/255.0,1));
		C2D_DrawImage(sprites[SPRITE_EMBLEM_ENDLESS].image,&sprites[SPRITE_EMBLEM_ENDLESS].params,NULL);
		break;
	default:
		C2D_DrawRectSolid(0,86,0,62,58,C2D_Color32f(1,51.0/255.0,0,1));
		break;
