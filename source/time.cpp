...
double get_current_time(int id) {
	...
	if ((id == 0 || id == 1) && get_isMusicStart() == true) { //メインのカウントの時はVorbis基準の時間を返す 要曲終了時の処理
		...
		CurrentTime[id] = Time[id] + (getVorbisTime() - IniVorbisTime[id])*0.001;
	...
	if (isStop[id] != 1) {
		...
		if (cnt[id] == 0) OffTime[id] = (float)(myTime.tv_sec + myTime.tv_usec * 0.000001);
		++cnt[id];
		Time[id] = (float)(myTime.tv_sec + myTime.tv_usec * 0.000001) - OffTime[id] + PreTime[id];
