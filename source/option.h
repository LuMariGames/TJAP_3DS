#pragma once

const char Text[3][64][128] = {
	{
		"ON",
		"OFF",
		"かんたん",
		"ふつう",
		"むずかしい",
		"おに",
		"おに裏",
		"太鼓タワー",
		"段位道場",
		"オート",
		"はやさ",
		"曲のはやさ",
		"ステルス",
		"あべこべ",
		"ランダム",
		"25%",
		"50%",
		"Lang.",
		"日本語",
		"English",
		"Español",
		"ﾊﾞｯﾌｧｻｲｽﾞ",
		"リセット",
		"fps表示",
		"連打速度",
		"はやさ(固定)",
		"ボタン割り当て",
		"スコア",
		"最大ｺﾝﾎﾞ数",
		"良",
		"可",
		"不可",
		"連打数",
		"STARTボタンを押して戻る",
		"演奏を続ける",
		"はじめからやり直す",
		"曲選択に戻る",
		"DSP1を起動していないため音は流れません",
		"音楽ファイルが読み込めません\n\nTJAファイルをUTF-8に変換しましたか？",
		"音楽ファイルをOgg(またはMp3)に変換して下さい",
		"TJAファイルを読み込めません",
		"譜面を正しく読めませんでした。\nSTARTボタンで曲選択画面に戻ります。",
		"不可の数",
		"可の数",
		"良の数",
		"スコア",
		"連打数",
		"叩けた数",
		"魂ゲージ",
		"以上",
		"未満",
		"とくしゅ",
		"かんぺき",
		"とっくん",
		"外部効果音",
		"ﾌﾟﾚｲﾔｰ(双打用)",
		"1P",
		"2P",
		"楽曲検索",
		"編集ﾓｰﾄﾞ",
	},
	{
		"ON",
		"OFF",
		"Easy",
		"Normal",
		"Hard",
		"Extreme",
		"Extra",
		"Tower",
		"Dan-i Dojo",
		"Auto",
		"Speed",
		"Music Speed",
		"Vanish",
		"Inverse",
		"Random",
		"25%",
		"50%",
		"Lang.",
		"日本語",
		"English",
		"Español",
		"BufferSize",
		"Reset",
		"Display fps",
		"RollSpeed",
		"Fixed Speed",
		"Button mapping",
		"Score",
		"MAX Combo",
		"GOOD",
		"OK",
		"BAD",
		"Drumroll",
		"Press START",
		"Continue",
		"Retry",
		"Back to Select song",
		"No sound is played because\nDSP1 is not activated.",
		"The sound file does not exist.\n\nHave you converted the character encoding\nof the TJA file to UTF8?",
		"The sound file is not an Ogg (or Mp3) file.",
		"The TJA file does not exist.",
		"Chart was not read correctly.\nPress START button to return to the song selection screen.",
		"Bads",
		"Nices",
		"Perfects",
		"Scores",
		"Rolls",
		"Hits",
		"Gauge",
		"more",
		"less",
		"special",
		"perfection",
		"Training",
		"External sound",
		"Player",
		"1P",
		"2P",
		"Song search",
		"Editor Mode",
	},
	{
		"Sí",
		"No",
		"Fácil",
		"Normal",
		"Difícil",
		"Extremo",
		"Extra",
		"torre",
		"Dan-i Dojo",
		"Auto",
		"Velocidad",
		"Canción velocidad",
		"Invisible",
		"Inversa",
		"Random",
		"25%",
		"50%",
		"Idioma",
		"日本語",
		"English",
		"Español",
		"Tamaño del búfer",
		"Reiniciar",
		"Mostrar fps",
		"Redoblevelocidad",
		"Velocidad fija",
		"Mapeo de botones",
		"Puntuación",
		"Combo máximo",
		"Bien",
		"OK",
		"Mal",
		"Redoble",
		"Presiona START",
		"Continuar",
		"Reintentar",
		"Volver al menú",
		"No sound is played because\nDSP1 is not activated.",
		"The sound file does not exist.\n\nHave you converted the character encoding\nof the TJA file to UTF8?",
		"The sound file is not an Ogg (or Mp3) file.",
		"The TJA file does not exist.",
		"No entendí el chart.\nPresiona START para regresar a la pantalla de selección.",
		"Mal",
		"OK",
		"bien",
		"Puntuación",
		"Redoble",
		"visitas",
		"indicador",
		"más que",
		"menos que",
		"especial",
		"perfección",
		"Capacitación",
		"Sonido externo",
		"Jugador",
		"1P",
		"2P",
		"Búsqueda de canciones",
		"Modo editor",
	}
};

typedef struct {

	int lang,buffer_size,Voice,special,player,measure;
	bool isAuto,isStelth,isSwap,dispFps,fixroll,exse,edit;
	float speed,random,offset,blacktext,musicspeed,
		judge_range_perfect,judge_range_nice,judge_range_bad;
	char SongTitle[256] = "\0";
	int KEY_A, KEY_B, KEY_DRIGHT, KEY_DLEFT, KEY_DUP, KEY_DDOWN, KEY_R, KEY_L, KEY_X, KEY_Y,
		KEY_ZL, KEY_ZR, KEY_CSTICK_RIGHT, KEY_CSTICK_LEFT, KEY_CSTICK_UP, KEY_CSTICK_DOWN,
		KEY_CPAD_RIGHT, KEY_CPAD_LEFT, KEY_CPAD_UP, KEY_CPAD_DOWN;
} OPTION_T;

enum Lang_knd {
	LANG_JP = 0,
	LANG_EN,
	LANG_ES,
};

enum KEY_KND {
	KEY_NONE = 0,
	KEY_DON,
	KEY_KATSU,
};

enum Text_knd {
	TEXT_ON,
	TEXT_OFF,
	TEXT_EASY,
	TEXT_NORMAL,
	TEXT_HARD,
	TEXT_ONI,
	TEXT_EDIT,
	TEXT_TOWER,
	TEXT_DAN,
	TEXT_AUTO,
	TEXT_SPEED,
	TEXT_MUSICSPEED,
	TEXT_STELTH,
	TEXT_SWAP,
	TEXT_RANDOM,
	TEXT_R25,
	TEXT_R50,
	TEXT_LANG,
	TEXT_JP,
	TEXT_EN,
	TEXT_ES,
	TEXT_BUFFERSIZE,
	TEXT_RESET,
	TEXT_DISP_FPS,
	TEXT_COMBO_VOICE,
	TEXT_FIXROLL,
	TEXT_BUTTON_MAPPING,
	TEXT_SCORE,
	TEXT_MAXCOMBO,
	TEXT_PERFECT,
	TEXT_NICE,
	TEXT_BAD,
	TEXT_ROLLCOUNT,
	TEXT_PRESSSTART,
	TEXT_CONTINUE,
	TEXT_STARTOVER,
	TEXT_RETURNSELECT,
	TEXT_WARNING_DSP1,
	TEXT_WARNING_WAVE_NO_EXIST,
	TEXT_WARNING_WAVE_NOT_OGG,
	TEXT_WARNING_TJA_NOT_EXIST,
	TEXT_WARNING_CHART_NOT_EXIST,
	TEXT_NUM_BAD,
	TEXT_NUM_NICE,
	TEXT_NUM_PERFECT,
	TEXT_NUM_SCORE,
	TEXT_NUM_ROLL,
	TEXT_NUM_HIT,
	TEXT_NUM_GAUGE,
	TEXT_NUM_UP,
	TEXT_NUM_DOWN,
	TEXT_SPECIAL,
	TEXT_KANPEKI,
	TEXT_TOKUN,
	TEXT_EXSE,
	TEXT_PLAYER,
	TEXT_1P,
	TEXT_2P,
	TEXT_SEARCH,
	TEXT_EDITOR_MODE,
};

int get_lang();
bool get_isauto(),get_ismeasure();
float mspeed();
void draw_option(u16 px, u16 py, unsigned int key , C2D_Sprite sprites[SPRITES_NUMER]),
toggle_auto(),get_option(OPTION_T *TMP),init_option(),load_option(),exit_option(),save_option();
