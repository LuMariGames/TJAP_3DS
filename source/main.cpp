...
bool loadPNGAsC2DImage(C2D_Image *texture, const char* filename, bool rgba, unsigned int width, unsigned int height, float img_x, float img_y) {
	...
	if (spng_decode_image(ctx, image, image_size, fmt, SPNG_DECODE_TRNS) != 0) {
		free(image);
		spng_ctx_free(ctx);
		fclose(f);
		return false;
...
int main(){
	...
	load_sprites();
	chartload = threadCreate(load_file_main,(void*)(""),16384,0x3f,-2,true);
	...
	while (aptMainLoop()){
		...
		case SCENE_MAINLOAD:	 //ロード中
			...
			C3D_FrameEnd(0);
			memtch_x=0,memtch_y=0,read_data.px=0,read_data.py=0;
			if (Option.edit == 1){
