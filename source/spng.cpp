...
#endif
{
	spng_ctx *ctx = (spng_ctx *)opaque; /* [FIX] void* -> spng_ctx* */
...
{
	spng_ctx *ctx = (spng_ctx *)opqaue; /* [FIX] void* -> spng_ctx* */
	spng__free(ctx, ptr);
}

static inline uint16_t read_u16(const void *src)
{
	const unsigned char *data = (const unsigned char *)src; /* [FIX] const void* -> const unsigned char* */
...
{
	unsigned char *data = (unsigned char *)dest; /* [FIX] void* -> unsigned char* */
...
static struct spng__iter spng__iter_init(unsigned bit_depth, const unsigned char *samples)
{
	...
	{
		.mask = (uint8_t)((1 << bit_depth) - 1), /* [FIX] narrowing conversion prevention */
...
{
	uint16_t *px = (uint16_t *)row; /* [FIX] void* -> uint16_t* */
	size_t i, n = size / 2;
